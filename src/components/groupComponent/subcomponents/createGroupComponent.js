...
export default function CreateGroupComponent() {
  ...
  const getSchedule = async () => {
    ...
    const hasInvalidValue = Object.values(groupData).some(
      (value) => value === "invalid" || value === "0"
    );
    if (hasInvalidValue) {
    ...
    for (const scheduleKey in groupData.groupSchedule) {
      ...
      if (
        ...
        for (let i = 0; i < schedule.endTime - startHour; i++) {
          ...
          indexLabels.push(
            schedulesArray[parseInt(scheduleKey) - 1].dictationTypeName
          );
        }
      }
    }
      ...
      return (
        selectedHours.indexes.findIndex(
          (otherHours, otherIndex) =>
            index !== otherIndex &&
            hours[0] === otherHours[0] &&
            hours[1] === otherHours[1]
        ) !== -1
      );
    });
  };
  ...
  const createGroup = async () => {
    ...
    if (verifySelectedHours()) {
      messageMediator.showMessage(
        "Los horarios seleccionados no son válidos...",
        "error"
      );
      return;
    }
    ...
    try {
      ...
      messageMediator.showMessage(
        "Se registro el grupo correctamente...",
        "success"
      );
      navigate("/group");
    } catch (error) {
      ...
      <div className={styles.groupDataContainer}>
        ...
        <div>
          ...
          <input
            ...
            min={0}
            max={100}
            placeholder={"Ingrese el tope del grupo..."}
            name={"limit"}
            onChange={handleChange}
            ...
            <div key={schedule.dictationTypeId} className={styles.scheduleMap}>
              <div>
                ...
                <input
                  type={"text"}
                  readOnly={true}
                  value={schedule.dictationTypeName}
                />
              </div>
              <div>
              ...
              <div>
                ...
                <select
                  name={`${schedule.dictationTypeId}-dayNumber`}
              ...
              <div>
                ...
                <input
                  type={"number"}
                  min={horaInicio}
                  max={horaFin - schedule.hours}
                  defaultValue={""}
                  name={`${schedule.dictationTypeId}-startTime`}
              ...
              <div>
                ...
                <input
                  value={groupSchedule[schedule.dictationTypeId]?.endTime || ""}
                  readOnly={true}
                />
              </div>
            </div>
