...
import FormToAssignmentSchedule from "./formToAssignmentSchedule";
...
class MessageFacade {
  ...
  openModalSemester = (semesters, fetchData) => {
    Swal.fire({
      ...
      html: ReactDOMServer.renderToString(
        <SemesterForm semesterOptions={semesters} />
      ),
      showCancelButton: true,
      ...
      preConfirm: (result) => {
        ...
        const selectedSemester = semesters.find(
          (semester) => semester.code === selectedSemesterCode
        );

        return axios
          .put(`/api/Semester/${selectedSemester.id}/Activate`, {})
          .then(() => {
            fetchData();
            return true;
          })
  ...
  openModalClassroom = (classroomsAvaibles, idSchedule) => {
    Swal.fire({
      title: "Asignar Aula",
      html: ReactDOMServer.renderToString(
        <ClassroomForm classroomOptions={classroomsAvaibles} />
      ),
      showCancelButton: true,
  ...
  openModalAssignmentSchedule = () => {
    Swal.fire({
      title: "Asignar Horario",
      html: ReactDOMServer.renderToString(<FormToAssignmentSchedule />),
